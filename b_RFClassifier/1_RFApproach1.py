...
def myRF_groupIDs(data, features, figname, foldername):
    ...
    # Test the ratio of pushing behavior in the train and test set, actually they are almost the same
    # ratioTrain=np.sum(y_train)/len(y_train)
    # ratioTest=np.sum(y_test)/len(y_test)
    # print(ratioTrain,ratioTest)
    ########################################
...
def RFmoti(data, N):
    print("INFO: start random forest classification on motivation level")
...
def plotWeights(weights, figname):
    ...
    N = int(len(weights[0]) / 4)
    for i in range(4):
        Heatmap(axes[i], weights[0][i * N:(i + 1) * N], featureType[i])
    plt.savefig(figname)
    plt.close()
    plt.cla()
    plt.clf()
...
if __name__ == '__main__':
    ...
    for folder in folders:
        ...
        RFall(alldata, N)
        os.chdir('{}/RFApproach1'.format(dir))
